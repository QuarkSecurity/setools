...
static int qpol_policy_build_attrs_from_map(qpol_policy_t * policy)
{
	...
	type_datum_t *new_type = NULL, *tmp_type, *orig_type;
	char *new_name = NULL, buff[10];
	int error = 0;
	...
	for (i = 0; i < db->p_types.nprim; i++) {
		...
		if (tmp_type == NULL) {
			/* first create a new type_datum_t for the
			 * attribute, with the attribute's type_list
			 * consisting of types with this attribute */
			...
			new_type = calloc(1, sizeof(type_datum_t));
			if (!new_type) {
				error = errno;
				goto err;
			}
			new_type->primary = 1;
			new_type->flavor = TYPE_ATTRIB;
			new_type->s.value = i + 1;
			int retv = hashtab_insert(db->p_types.table, (hashtab_key_t) new_name, (hashtab_datum_t) new_type);
			if (retv) {
			...
			db->p_type_val_to_name[i] = new_name;
			db->type_val_to_struct[i] = new_type;
			tmp_type = new_type;
