...
int main(int argc, char **argv)
{
	...
	TEST("opening the binary policy file",
	     !bin_open_policy(pol_path, &p));
...
int type_query(apol_policy_t * p)
{
	...
	printf
	    ("============================================ QUERY TYPES ==========================================\n\n\n");
	TEST("querying the policy for all types",
	     !apol_get_type_by_query(p, NULL, &v));
	if (v == NULL) {
		...
		printf
		    ("Got queries out of apol_get_type_by_query...PASS\n");
	}
	vector_size = apol_vector_get_size(v);
	printf("the size of the apol_vector structure passed in is %d\n",
	       vector_size);
	printf
	    ("printing all the names of each of the datums in the vector\n");

	for (n = 0; n < vector_size; n++) {
		printf("item %d: ", n);
		type_datum_ptr =
		    (sepol_type_datum_t *) apol_vector_get_element(v, n);
		sepol_type_datum_get_name(p->sh, p->p, type_datum_ptr,
					  &name);
		printf("%s\n", name);
	}
	...
	printf("\n\n---------------GET dir_t TYPE---------------\n");
	TEST("creating a new apol_type_query_t structure t",
	     (t = apol_type_query_create()));
	TEST("setting the apol_type_query_t structure to type dir_t",
	     !apol_type_query_set_type(p, t, "dir_t"));
	...
	TEST("creating a new apol_type_query_t structure t",
	     (t = apol_type_query_create()));
	TEST("unsetting my type_query_t structure to NULL name",
	     !apol_type_query_set_type(p, t, NULL));
	printf
	    ("\n\n---------------GET REGEX \"se\" TYPE---------------\n");
	TEST("setting the type_query_t structure to use regex",
	     !apol_type_query_set_regex(p, t, 1));
	TEST("setting the apol_type_query_t structure to regex se",
	     !apol_type_query_set_type(p, t, "se"));
	TEST("querying the policy with the newly set apol_type_query_t structure", !apol_get_type_by_query(p, t, &v));
	...
	printf
	    ("\n\n---------------GET REGEX \"ex\" TYPE---------------\n");
	TEST("setting this query structure to do regex",
	     !apol_type_query_set_regex(p, t, 1));
	TEST("setting the apol_type_query_t structure to regex ex",
	     !apol_type_query_set_type(p, t, "ex"));
	TEST("querying the policy with the newly set apol_type_query_t structure", !apol_get_type_by_query(p, t, &v));
...
int attribute_query(apol_policy_t * p)
{
	...
	TEST("querying the policy for all attributes",
	     !apol_get_attr_by_query(p, NULL, &v));
	...
	if (vector_size == 0) {
		...
		for (n = 0; n < vector_size; n++) {
			...
			type_datum_ptr =
			    (sepol_type_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_type_datum_get_name(p->sh, p->p,
						  type_datum_ptr, &name);
			sepol_type_datum_get_name(p->sh, p->p,
						  (sepol_type_datum_t *)
	...
	TEST("creating an apol attribute query structure",
	     (attr_s = apol_attr_query_create()) != NULL);
	TEST("setting attribute name to \"random\" in the attribute query structure", !apol_attr_query_set_attr(p, attr_s, "random"));
	TEST("doing an attribute query with attribute name = \'random\'",
	     !apol_get_attr_by_query(p, attr_s, &v));
	vector_size = apol_vector_get_size(v);
	...
	if (v == NULL) {
		...
		printf
		    ("The vector after get attribute is not NULL...PASS\n");
	}
	...
	TEST("setting the attribute query structure to be regex'ed",
	     !apol_attr_query_set_regex(p, attr_s, 1));
	TEST("setting the regex of the query structure",
	     !apol_attr_query_set_attr(p, attr_s, "re*"));
	TEST("re'querying the policy with regex attribute structure",
	     !apol_get_attr_by_query(p, attr_s, &v));
	if (v == NULL) {
		fprintf(stderr,
			"v returned NULL, get attribute by query failed\n");
...
int role_query(apol_policy_t * p)
{
	...
	printf("============================================ QUERY ROLES "
	       "==========================================\n\n\n");
	TEST("querying the policy for all roles",
	     !apol_get_role_by_query(p, NULL, &v));
	if (v == NULL) {
		fprintf(stderr,
			"the vector is NULL, either no results or error");
	} else {
	...
	for (n = 0; n < vector_size; n++) {
		printf("item %d: ", n);
		role_datum_ptr =
		    (sepol_role_datum_t *) apol_vector_get_element(v, n);
		sepol_role_datum_get_name(p->sh, p->p, role_datum_ptr,
					  &name);
		printf("%s\n", name);
	}
	printf("destroying all the elements in v\n");
	...
	TEST("setting a role name in the apol role query structure",
	     !apol_role_query_set_role(p, query_s_role, "user_r"));
	TEST("re-querying the policy for the specified role",
	     !apol_get_role_by_query(p, query_s_role, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr,
			"There were no roles with name role_r found\n");
	} else {
		for (n = 0; n < vector_size; n++) {
			printf("item %d: ", n);
			role_datum_ptr =
			    (sepol_role_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_role_datum_get_name(p->sh, p->p,
						  role_datum_ptr, &name);
			printf("%s\n", name);
		}
	}
...
int user_query(apol_policy_t * p)
{
	...
	printf("============================================ QUERY USERS "
	       "==========================================\n\n\n");
	TEST("querying the policy for all users",
	     !apol_get_user_by_query(p, NULL, &v));
	if (v == NULL) {
		fprintf(stderr,
			"the vector is NULL, either no results or error");
	} else {
	...
	for (n = 0; n < vector_size; n++) {
		printf("item %d: ", n);
		user_datum_ptr =
		    (sepol_user_datum_t *) apol_vector_get_element(v, n);
		sepol_user_datum_get_name(p->sh, p->p, user_datum_ptr,
					  &name);
		printf("%s\n", name);
	}
	printf("destroying all the elements in v\n");
	...
	TEST("setting a role name in the apol role query structure",
	     !apol_user_query_set_user(p, user_s, "joe"));
	TEST("re-querying the policy for the specified user",
	     !apol_get_user_by_query(p, user_s, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr,
			"There were no roles with name role_r found\n");
		...
		for (n = 0; n < vector_size; n++) {
			...
			user_datum_ptr =
			    (sepol_user_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_user_datum_get_name(p->sh, p->p,
						  user_datum_ptr, &name);
			printf("%s\n", name);
		}
	}
	...
	printf
	    ("\n\n---------------GET REGEX USER \"ro*\"---------------\n");
	TEST("setting the user query structure to be regex'ed",
	     !apol_user_query_set_regex(p, user_s, 1));
	TEST("setting the user query strcuture reg ex",
	     !apol_user_query_set_user(p, user_s, "syst*"));
	TEST("re-querying the policy for the specified user",
	     !apol_get_user_by_query(p, user_s, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting role to user query structure",
	     !apol_user_query_set_role(p, user_s, "system_r"));
	TEST("re-querying the policy for the specified user",
	     !apol_get_user_by_query(p, user_s, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
	...
	if (apol_policy_is_mls(p)) {
		...
		mls_v_high =
		    apol_mls_level_create_from_string(p, "s9:c0.c127");
		if (mls_v_high == NULL) {
		...
		TEST("setting the high level of the mls range structure",
		     !apol_mls_range_set_high(p, mls_range_var,
					      mls_v_high));

		TEST("calling apol_user_query_set_range on user query structure", !apol_user_query_set_range(p, user_s, mls_range_var, APOL_QUERY_EXACT));
		TEST("re-querying the policy for the specified user",
		     !apol_get_user_by_query(p, user_s, &v));
		vector_size = apol_vector_get_size(v);
		if (vector_size == 0) {
...
int classes_query(apol_policy_t * p)
{
	...
	TEST("querying policy for all object classes",
	     !apol_get_class_by_query(p, NULL, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
		for (n = 0; n < vector_size; n++) {
			...
			class_datum_ptr =
			    (sepol_class_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_class_datum_get_name(p->sh, p->p,
						   class_datum_ptr, &name);
			printf("%s\n", name);
		}
	}
	apol_vector_destroy(&v, NULL);
	printf("--------LISTING OBJECT CLASS \"process\"--------\n");
	TEST("creating a class query structure",
	     (query_s_class = apol_class_query_create()) != NULL);
	TEST("assigning class query structure a name",
	     !apol_class_query_set_class(p, query_s_class, "process"));
	TEST("querying the policy for object class \"process\"",
	     !apol_get_class_by_query(p, query_s_class, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
	...
	TEST("setting the class query structure to have regex",
	     !apol_class_query_set_regex(p, query_s_class, 1));

	TEST("assigning class query structure a regex",
	     !apol_class_query_set_class(p, query_s_class, "fi"));

	TEST("querying the policy for class",
	     !apol_get_class_by_query(p, query_s_class, &v));
	...
	apol_vector_destroy(&v, NULL);
	printf
	    ("--------LISTING OBJECT CLASSES THAT INHERIT FROM  \"socket\"--------\n");
	TEST("setting query class to have no regex",
	     !apol_class_query_set_regex(p, query_s_class, 0));
	TEST("unsetting name in query class",
	     !apol_class_query_set_class(p, query_s_class, NULL));
	TEST("setting query class common",
	     !apol_class_query_set_common(p, query_s_class, "ipc"));
	TEST("querying the policy for class",
	     !apol_get_class_by_query(p, query_s_class, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
...
int common_classes_query(apol_policy_t * p)
{
	...
	apol_vector_t *v;
	printf
	    ("============================================ QUERY COMMON CLASSES "
	     "==========================================\n\n\n");
	printf("--------LISTING ALL COMMON CLASSES--------\n");
	TEST("querying the policy for all the common classes",
	     !apol_get_common_by_query(p, NULL, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
		for (n = 0; n < vector_size; n++) {
			printf("item %d: ", n);
			common_datum_ptr =
			    (sepol_common_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_common_datum_get_name(p->sh, p->p,
						    common_datum_ptr,
						    &name);
			printf("%s\n", name);
		}
	...
	TEST("creating a common query structure",
	     (query_s_common = apol_common_query_create()) != NULL);
	TEST("setting the common structure with common name \"ipc\"",
	     !apol_common_query_set_common(p, query_s_common, "ipc"));

	TEST("querying the policy for all the common classes",
	     !apol_get_common_by_query(p, query_s_common, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
...
int permissions_query(apol_policy_t * p)
{
	...
	TEST("query policy for all permissions",
	     !apol_get_perm_by_query(p, NULL, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
	...
	TEST("creating the permission query structure",
	     (query_s_perm = apol_perm_query_create()) != NULL);
	TEST("setting the permission query structure with permission name \"bind\"", !apol_perm_query_set_perm(p, query_s_perm, "bind"));
	TEST("querying the policy with the permission structure",
	     !apol_get_perm_by_query(p, query_s_perm, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
	...
	TEST("setting the permission structure to be regex'ed",
	     !apol_perm_query_set_regex(p, query_s_perm, 1));

	TEST("setting the regex of the permission structure to be \"ge\"",
	     !apol_perm_query_set_perm(p, query_s_perm, "ge"));
...
int portcon_query(apol_policy_t * p)
{
	...
	TEST("query policy for all portcons",
	     !apol_get_portcon_by_query(p, NULL, &v));
	...
	if (vector_size == 0) {
		...
		for (n = 0; n < vector_size; n++) {
			printf("port %d: ", n);
			portcon_tmp_p =
			    (sepol_portcon_t *) apol_vector_get_element(v,
									n);
			sepol_portcon_get_low_port(p->sh, p->p,
						   portcon_tmp_p, &port_p);
			printf("%d\n", port_p);
		}
	}
	...
	if (vector_size == 0) {
		...
		for (n = 0; n < vector_size; n++) {
			...
			sepol_portcon_get_context(p->sh, p->p,
						  portcon_tmp_p,
						  &context_p);
			sepol_context_struct_get_user(p->sh, p->p,
						      context_p,
						      &user_datum_p);
			sepol_user_datum_get_name(p->sh, p->p,
						  user_datum_p, &name);
			sepol_context_struct_get_role(p->sh, p->p,
						      context_p,
						      &role_datum_p);
			sepol_role_datum_get_name(p->sh, p->p,
						  role_datum_p,
						  &role_name);
			sepol_context_struct_get_type(p->sh, p->p,
						      context_p,
						      &type_datum_p);
			sepol_type_datum_get_name(p->sh, p->p,
						  type_datum_p,
	...
	TEST("creating an apol portcon structure",
	     (apol_portcon_query_p = apol_portcon_query_create()));

	TEST("setting the protocol of the portcon structure",
	     !apol_portcon_query_set_proto(p, apol_portcon_query_p, 6));
	TEST("setting the low port of the portcon structure",
	     !apol_portcon_query_set_low(p, apol_portcon_query_p, 21));
	TEST("setting the high port of the portcon structure",
	     !apol_portcon_query_set_high(p, apol_portcon_query_p, 21));

	if (context_p != NULL) {
		TEST("creating an apol context struct from an old sepol context struct", (apol_context_p = apol_context_create_from_sepol_context(p, context_p)));
	} else {
		TEST("creating an apol context from scratch",
		     (apol_context_p = apol_context_create()));
		TEST("setting the user of the context structure",
		     !apol_context_set_user(p, apol_context_p,
					    "system_u"));
		TEST("setting the role of the context structure",
		     !apol_context_set_role(p, apol_context_p,
					    "object_r"));
		TEST("setting the type of the context structure",
		     !apol_context_set_type(p, apol_context_p,
					    "net_foo_t"));
		TEST("creating an mls range",
		     (apol_mls_range_p = apol_mls_range_create()));
		TEST("creating low level",
		     (apol_mls_low_lvl_p =
		      apol_mls_level_create_from_string(p, "s1:c0.c2")));
		TEST("creating high level",
		     (apol_mls_high_lvl_p =
		      apol_mls_level_create_from_string(p, "s2:c0.c4")));
		TEST("setting the low level of the range",
		     !apol_mls_range_set_low(p, apol_mls_range_p,
					     apol_mls_low_lvl_p));
		TEST("setting the high level of the range",
		     !apol_mls_range_set_high(p, apol_mls_range_p,
					      apol_mls_high_lvl_p));
		TEST("setting the range of the context structure",
		     !apol_context_set_range(p, apol_context_p,
					     apol_mls_range_p));
	}

	TEST("setting the context of the portcon structure",
	     !apol_portcon_query_set_context(p, apol_portcon_query_p,
					     apol_context_p,
					     APOL_QUERY_EXACT));

	TEST("query policy using the created portcon query structure",
	...
	printf
	    ("-------- LISTING PORT CONTEXTS THAT MATCH THE QUERY --------\n");
	...
	printf
	    ("-------- QUERYING FOR PORTS WITH SLIGHTLY CHANGED CONTEXT --------\n");
	...
	TEST("setting the user of the context structure",
	     !apol_context_set_user(p, apol_context_p, context_user));
	TEST("setting the role of the context structure",
	     !apol_context_set_role(p, apol_context_p, context_role));
	TEST("setting the type of the context structure",
	     !apol_context_set_type(p, apol_context_p, context_type));
	...
	TEST("setting the high level of the range",
	     !apol_mls_range_set_high(p, apol_mls_range_p,
				      apol_mls_high_lvl_p));
	TEST("setting the range of the context structure",
	     !apol_context_set_range(p, apol_context_p, apol_mls_range_p));

	TEST("query policy second time with same portcon query structure",
	     !apol_get_portcon_by_query(p, apol_portcon_query_p, &v));
		...
		for (n = 0; n < vector_size; n++) {
			...
			TEST("",
			     !sepol_context_struct_get_user(p->sh, p->p,
							    context_p,
							    &user_datum_p));
			TEST("getting the name from the user datum",
			     !sepol_user_datum_get_name(p->sh, p->p,
							user_datum_p,
							&name));
			TEST("",
	...
	TEST("setting the user of the context structure",
	     !apol_context_set_user(p, apol_context_p, "random_u"));

	if (!strcmp(pol_path, DEF_NON_MLS_POL)) {
		TEST("query policy second time with same portcon query structure (this call fails)", apol_get_portcon_by_query(p, apol_portcon_query_p, &v));
	} else {
		TEST("query policy second time with same portcon query structure (this call fails)", !apol_get_portcon_by_query(p, apol_portcon_query_p, &v));
	}
...
int netifcon_query(apol_policy_t * p)
{
	...
	TEST("creating a apol netifcon query structure",
	     (apol_netifcon_query_p = apol_netifcon_query_create()));
	...
	TEST("setting the interface messages context",
	     !apol_netifcon_query_set_msg_context(p,
						  apol_netifcon_query_p,
						  apol_context_p,
						  APOL_QUERY_EXACT));
	...
	if (vector_size == 0) {
		...
		for (n = 0; n < vector_size; n++) {
			printf("netif %d: ", n);
			sepol_netifcon_p =
			    (sepol_netifcon_t *) apol_vector_get_element(v,
									 n);
			sepol_netifcon_get_name(p->sh, p->p,
						sepol_netifcon_p,
						&netifcon_name);
			printf("%s\n", netifcon_name);
		}
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting the interface messages context",
	     !apol_netifcon_query_set_msg_context(p, apol_netifcon_query_p,
						  NULL, APOL_QUERY_EXACT));


	TEST("setting the netifcon query structure's device to \"lo\"",
	     !apol_netifcon_query_set_device(p, apol_netifcon_query_p,
					     "lo"));
	...
	printf
	    ("-------- LISTING NETIFCONS WITH INTERFACE NAME \"lo\"  --------\n");
	...
	printf
	    ("-------- DOING ANOTHER QUERY, STRUCTURE ONLY HAS DEVICE SET TO \"lo1\" --------\n");
	TEST("setting the netifcon query structure's device to \"l01\"",
	     !apol_netifcon_query_set_device(p, apol_netifcon_query_p,
					     "lo1"));

	TEST("query policy with query structure",
	     !apol_get_netifcon_by_query(p, apol_netifcon_query_p, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	...
	apol_vector_destroy(&v, NULL);
	printf
	    ("-------- DOING QUERY WITH DEVICE \"lo\" AND CONTEXT \"system_u:object_r:netif_lo_t\" --------\n");
	TEST("setting the netifcon query structure's device to \"lo\"",
	     !apol_netifcon_query_set_device(p, apol_netifcon_query_p,
					     "lo"));
	TEST("creating an apol context from scratch",
	...
	TEST("querying netifcons based on a context and a netifcon query structure", !apol_netifcon_query_set_if_context(p, apol_netifcon_query_p, apol_context_p, APOL_QUERY_EXACT));
	...
	TEST("setting the netifcon query structure's device to \"eth1\"",
	     !apol_netifcon_query_set_device(p, apol_netifcon_query_p,
					     "eth1"));
	TEST("querying netifcons based on a context and a netifcon query structure", !apol_netifcon_query_set_if_context(p, apol_netifcon_query_p, NULL, APOL_QUERY_EXACT));
	TEST("query policy all netifcons (NULL)",
	     !apol_get_netifcon_by_query(p, apol_netifcon_query_p, &v));
	...
	if (strcmp(pol_path, DEF_MLS_POL) == 0) {
		TEST("unsetting the netifcon's device to \"NULL\"",
		     !apol_netifcon_query_set_device(p,
						     apol_netifcon_query_p,
						     NULL));
		TEST("creating an apol context from scratch",
		     (apol_context_p = apol_context_create()));
		printf
		    ("I'M IN MLS POLICY AND DOING MSG CONTEXT!!!!!!!!!!!!!!!!\n");
		...
		TEST("query policy all netifcons with matching msg contexts", !apol_get_netifcon_by_query(p, apol_netifcon_query_p, &v));
...
static int category_query(apol_policy_t * p)
{
	...
	if (vector_size == 0) {
		...
		for (n = 0; n < vector_size; n++) {
			...
			cat_datum_p =
			    (sepol_cat_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_cat_datum_get_name(p->sh, p->p, cat_datum_p,
						 &cat_name);
			printf("%s\n", cat_name);
		}
	}
	apol_vector_destroy(&v, NULL);
	TEST("creating a category structure",
	     !((apol_cat_query_p = apol_cat_query_create()) == NULL));
	TEST("setting the name of the category query",
	     !apol_cat_query_set_cat(p, apol_cat_query_p, "c9"));
	...
	TEST("setting the category name to \"c245\"",
	     !apol_cat_query_set_cat(p, apol_cat_query_p, "c245"));
...
int levels_query(apol_policy_t * p)
{
	...
	printf
	    ("\n============================================ QUERY LEVELS "
	     "==========================================\n\n\n");
	TEST("getting all levels defined in the policy",
	     !apol_get_level_by_query(p, NULL, &v));
	vector_size = apol_vector_get_size(v);
	if (vector_size == 0) {
		fprintf(stderr, "vector size is 0, no results\n");
	} else {
		for (n = 0; n < vector_size; n++) {
			...
			se_lvl_datum_p =
			    (sepol_level_datum_t *)
			    apol_vector_get_element(v, n);
			sepol_level_datum_get_name(p->sh, p->p,
						   se_lvl_datum_p,
						   &level_name);
			printf("%s\n", level_name);
		}
	}
	TEST("creating a level query structure",
	     (ap_lvl_query_p = apol_level_query_create()));
	TEST("setting the sensitivity of the level structure to s8",
	     !apol_level_query_set_sens(p, ap_lvl_query_p, "s8"));
	apol_vector_destroy(&v, NULL);
	...
	TEST("setting the sensitivity of the level structure to s56",
	     !apol_level_query_set_sens(p, ap_lvl_query_p, "s56"));
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting the level to use regex",
	     !apol_level_query_set_regex(p, ap_lvl_query_p, 1));
	TEST("setting the category to regex \"c*\"",
	     !apol_level_query_set_cat(p, ap_lvl_query_p, "c"));
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting the category to regex \"s*\"",
	     !apol_level_query_set_cat(p, ap_lvl_query_p, "u"));
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting the category NULL",
	     !apol_level_query_set_cat(p, ap_lvl_query_p, NULL));
	TEST("setting the category to regex \"s*\"",
	     !apol_level_query_set_sens(p, ap_lvl_query_p, "s"));
	...
	apol_vector_destroy(&v, NULL);
	TEST("setting the category to regex \"s*\"",
	     !apol_level_query_set_sens(p, ap_lvl_query_p, "p"));
