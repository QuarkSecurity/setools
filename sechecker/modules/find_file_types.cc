...
namespace sechk
{
	...
	find_file_types_module::find_file_types_module(const find_file_types_module & rhs)
	:module(rhs)
	{
		// nothing more to do
	}
	...
	void find_file_types_module::run_internal(apol_policy_t * pol, sefs_fclist * list) throw (std::runtime_error)
	{
		...
		if (_recommendations.at(require_code_name(SECHK_REQUIRE_POLICY_CAPABILITY_ATTRIBUTE_NAMES)).check(pol, list))
		{
			...
			polsearch_string_expression_parameter * attr_names = new polsearch_string_expression_parameter(_options.at("file_type_attribute").values());
