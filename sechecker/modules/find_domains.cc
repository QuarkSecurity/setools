...
namespace sechk
{
	find_domains_module::find_domains_module() throw(std::invalid_argument, std::out_of_range)
	:module("find_domains", SECHK_SEV_UTIL, "Find all types treated as a domain.",
	...
	find_domains_module::find_domains_module(const find_domains_module & rhs)
	:module(rhs)
	{
		//nothing more to do.
	}
	...
	void find_domains_module::run_internal(apol_policy_t * pol, sefs_fclist * list) throw(std::runtime_error)
	{
		...
		{
			const char * tmp = NULL;
			qpol_role_get_name(apol_policy_get_qpol(pol), static_cast<qpol_role_t*>(apol_vector_get_element(v, i)), &tmp);
