...
namespace sechk
{
	roles_wo_users_module::roles_wo_users_module() throw(std::invalid_argument, std::out_of_range)
	:module("roles_wo_users",SECHK_SEV_LOW,"Find roles not assigned to a user.",
	"Roles not assigned to a user cannot form a valid context.")
	{
		// nothing more to do
	}

	roles_wo_users_module::roles_wo_users_module(const roles_wo_users_module & rhs)
	:module(rhs)
	{
		// nothing more to do
	}
