...
namespace sechecker
{
	...
	class module
	{
		 ...
		 * @post All of this module's requirements and reccommendations will be updated.
		 */
		 ...
		 * of each of its requirements, reccommendations, and options.
		 ...
		 */
		const option & setOption(const std::string & name, const std::vector < std::string > &values,
		...
		/**
		 * Get the set of reccommended features that a policy (or
		 * associated file context list) should meet to obtain the most
		 * complete set of results possible for the module.
		 * @return The set of reccommendations.
		 */
		const std::map < std::string, requirement > &reccommendations() const;
		...
		friend class sechecker;
	      ...
	      protected:
		const sechecker *owner;	//!< The sechecker object that owns this module.
		...
		severity _sev;	       //!< The severity level of this module's results.
		 ...
		 std::map < std::string, requirement > _reccommendations;	//!< The set of all features that are recommended for more complete results but not required to call run().
		 std::map < std::string, option > _options;	//!< The configurable options available to this module.
		bool _selected;	       //!< Set during dependency resolution to indicate that the module needs to be run.
